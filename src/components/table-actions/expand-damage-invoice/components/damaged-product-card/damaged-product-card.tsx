...
import { DamageFeedback } from "..";
import { cn } from "@/lib/utils";
import { useTranslation } from "react-i18next";
import { formatCounts, formatNumber, handleDirectionChange, type Language } from "@/localization";
import type { DamagedProduct } from "@/types/api-types";

interface DamagedProduct_Card_Props {
  product: DamagedProduct;
}

const baseUrl = import.meta.env.VITE_API_BASE_URL + "StaticFiles/Images/";

export default function DamagedProduct_Card({ product }: DamagedProduct_Card_Props) {
  const { t, i18n } = useTranslation();
  const { productId, productName, productType, quantity, damageDescription, damagedImages } = product;
  ...
  return (
    <div className="relative border rounded-lg p-2">
      ...
      <DamageFeedback reason={damageDescription} damageImages={damagedImages} className="mt-2" />
